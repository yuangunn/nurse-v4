...
- **병상 단위 배정 (2026-09-18, v260918i, 결정 2-26)**: 환자가 많으면 **한 병실을 둘이 나눠 본다**. 토큰은 **방**(`1001`) 또는 **병상**(`1001:1`)이고
  ...
  **코어에는 늘 병상 키로 펼쳐서 넘긴다**(`roomsForCore` → `expandBeds`) — 방 표기와 병상 표기가 섞이면 같은 환자를 가리키는 두 토큰이 문자열로 달라
  이어짐 겹침이 0이 된다. 펼쳐 넘기면 겹침이 곧 **같이 보던 환자 수**라 `bedsOf` 는 병상 키마다 1. **`assign-core.js` 는 손대지 않았다** — 코어는 토큰을
  문자열로만 다루므로 원칙 등급·가중치·라벨 매칭이 그대로다(회귀 `test_assign_core.mjs` 에 5인실을 3+2 로 나눈 시나리오와 5→4인 전환 시나리오가 있다).
  헬퍼는 `splitBed`/`bedTok`/`bedCount`/`bedKeys`/`expandBeds`/`collapseBeds`(다 찬 방은 방 하나로 접기)/`roomsLabel`(표기+단위 — 병상이 섞이면 '호'를 빼고 `4:2`).
  입력은 `4:1~3`·`4:1~4:3`·`4:1, 4:4`, 픽커는 방 버튼 옆 **⋮** 로 병상을 펼쳐 고른다(주지 않을 방도 병상 단위). 검사(`presetAudit`·`dupRooms`)와
  '아무도 안 보는 방'은 전부 병상 단위 — 안 그러면 나눠 본 방이 통째로 '빠진 방'으로 잡힌다. 병상수를 줄이면 없어진 병상 토큰을 걷어내고,
  병동을 바꾸면 병상 번호는 그대로 둔 채 방만 같은 자리로 옮긴다. 인쇄는 길이별로 글자를 줄이고 **쉼표에서만 접는다**(101·122 공통).
- 인트라넷용 ①: `standalone/assign.html` — 단일 파일 통합본(외부 의존 0, Chrome/Edge 103+).
  ...
  **처음 시작 = 설정 마법사 7단계** (`WIZ`/`startWizard`) — 병동 → 방 구성 → 요일별 인원
  → 근무표 → 모르는 표기 → 간호사·가능 근무 → 끝. 각 단계는 **기존 관리 패널을 그대로
  품는다**(`ADMIN_PANELS[id].body()/after()`) — 화면을 새로 만들면 두 벌이 갈라진다.
  붙여넣기만 전체 화면을 빌렸다가 `confirmPaste` → `wizResume()` 로 돌아온다.
  **화면 상태는 데이터 파일에** (`store.ui` — 인쇄함·시작안내 숨김·처음안내 봄).
  병원 PC 는 브라우저를 닫을 때 사이트 데이터를 지우는 경우가 많아 localStorage 에
  두면 매일 아침 풀린다. 테마·마지막 연 파일만 localStorage 에 남긴다.
  ...
  **배정표 서식 3벌** (관리 > 배정표 양식에서 고른다, 데이터 파일에 저장 — 자동 판별 안 함):
  `101`(요일마다 방·이름 두 칸) · `122`(이름·메모 두 칸 + 방은 일/월~금/토 세 열) ·
  `102`(**방 칸 없음** — 자리마다 보는 방이 고정이라 머리글에 한 번 적는다. "배정표에
  어싸인이 안 나오게" 라는 요구가 양식에 이미 들어 있다. 인쇄는 안 만들었고 xlsx 로
  내보내 엑셀에서 인쇄한다). 서식 추가는 `FORMS` + `findLayoutNNN` + `fillFormNNN`
  + 빌드 마커 `TEMPLATENNN_B64` 네 곳.
  **양식 파일은 병동이 준 것을 바이트 그대로 쓴다** — openpyxl 로 다시 저장하면
  머리글 그림(102 의 CPR 업무분담 `xl/media/image1.png`)이 사라진다. 그리고 같은
  서식이어도 판마다 칸 위치가 다르므로(102 실례: 요일 칸 1칸↔2칸 병합, 자리 열 B↔C,
  교육·행사 줄 위↔아래, 대체 줄 유무) `findLayoutNNN` 은 **위치를 가정하지 말고 훑어서
  찾을 것**. 화면과 인쇄가 어긋나면 화면에서 고친 자리와 인쇄된 자리가 달라진다.
  **차지 자리를 서식에 못 박지 말 것** — 102 는 차지 방이 고정이 아니라 그 근무 최선임
  이름 뒤에 `/CRN` 만 붙인다(2026-09-20 병동 확인). 자리를 옮기는 규칙을 넣었다가 걷어냈다.
  자리 이름에 CN 표기가 없는 서식은 화면에도 `/CRN` 을 보여야 누가 차지인지 안다.
  **양식을 갈아 끼우면 `python3 scripts/check_form_borders.py` 를 돌릴 것** (CI 에도 있다) —
  한 칸만 서식이 어긋나도 그 자리만 선이 빠진 채 매주 인쇄된다. 122 H18(월↔화 경계)이
  그랬고 쓰다가 사람이 알아챘다. 구멍(양쪽 다 선 없음)은 실패, '가늘다'는 안내만 한다.
  맥은 한글 파일 이름을 NFD 로도 저장하므로 한글 패턴 glob 은 조용히 빠뜨린다 — 정규화할 것.
  **화면 오버레이(`position:fixed`)는 반드시 `body` 직속에 둘 것** — 화면 컨테이너 안에 두면
  다른 화면일 때 `display:none` 에 딸려 0×0 이 되어, 열려 있는데도 안 보인다(`#wiz` 가 그랬다).
  **패널을 고치는 조작은 `renderAdmin()` 을 부를 것** — 마법사가 같은 패널을 제 본문에
  그려 쓰므로 거기서 마법사까지 함께 갱신한다. DOM 을 직접 만지면 마법사에서만 안 바뀐다.
  `scripts/test_assign_e2e.mjs` 가 이 둘을 실제 클릭으로 지킨다(CI).
  **다른 소속(82병동 SU)**: 배정표에 제 칸이 따로 있는 소속은 `store.unit[이름]` 으로
  표시하고 **병동 배정에서 뺀다**(`recompute` 의 `wardOnly`). 붙여넣을 때 날짜 줄이 한 번 더
  나오면 거기서부터 다른 표로 보고 제목의 괄호(`…(SU)`)를 소속 이름으로 쓴다 — 자동으로
  가르되 간호사 관리의 칩으로 고칠 수 있다. 자리 구성은 `FORMS[*].slots` 가 정한다
  (`['차지','A','SU','중간번']` 처럼 앱 자리가 아닌 칸이 섞일 수 있다).
  **병동과 서식은 한 쌍** — `setWard` 와 `setFormId` 가 서로를 부른다(무한 되풀이는 두 번째
  인자로 끊는다). 그 병동 양식이 없으면 서식은 건드리지 않는다.
  **내장 서식은 4벌, 병동은 14개** — `store.forms[병동코드]` 에 올린 양식을 넣으면
  `wardForm()` 이 서식 목록에 자리를 만든다. 모양은 `detectFormShape` 가 구조로 읽는다
  (방 열 3개=122 · 구역 줄 병합 여부로 101↔102 · SU 자리=82 · 날짜 칸이 글자면 글자).
  새 서식을 손으로 추가할 때도 이 판별기가 같은 답을 내는지 확인할 것 — 82 로 대조했다.
  **붙여넣기는 `parseClipboard()` 로만 읽을 것** — 엑셀은 줄바꿈이 든 칸을 큰따옴표로 감싼다.
  `split('\n')` 하면 표가 찢어진다(82 번표 42행 → 74줄). **검증은 사용자가 실제로 하는 동작으로** —
  격자를 `ingestGrid` 에 직접 넣어 통과시킨 SU 분리가 실제 붙여넣기에선 안 됐다.
  **다른 소속은 명부 뒤쪽에 모은다**(`groupUnits`) — 이 불변식 덕에 화면마다 정렬하지 않는다.
  인원을 세는 곳(그리드 합계·필요 인원 역산·쉬는 사람 후보)은 `unitOfName(n)` 이면 건너뛸 것.
  **테스트·하네스의 `ev(\`…\`)` 안에서는 정규식에 `\d` `\b` `\n` 을 쓰지 말 것** — 템플릿 리터럴이
  먼저 먹어 `d`·백스페이스·진짜 줄바꿈이 된다(세 번 걸렸다). `[0-9]`, `classList` 를 쓴다.
