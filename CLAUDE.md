...
- 인트라넷용 ①: `standalone/assign.html` — 단일 파일 통합본(외부 의존 0, Chrome/Edge 103+).
  ...
  **실사용 전수조사 반영 (2026-09-17, v260917j)**: 코드 정독 + 실제 규모(18명·두 달치·92병동) 워크스루로 찾은 24건과 사용자 추가 요청 2건을 한 번에 반영.
  ...
  ⑤ **관리**: `store.has13`(병실 목록 [13호 있음] — 병동마다 13호가 있기도 없기도 해 사용자가 고른다. `wardRooms(code,has13)`, 켜면 12호 보는 자리에 `attach13`, 끄면 병실·프리셋·금지 방에서 제거;
  `migrateStore` 는 병실에 13호가 있는데 어느 프리셋도 모르면 붙인다 — 92병동 63호를 아무도 안 보던 빈틈). **기본값은 13호 없음**(기존 기본 병실 목록과 같음). 방 구성 `presetAudit/presetAuditHtml`
