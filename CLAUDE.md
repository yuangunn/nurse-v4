...
3. **오프특근**: 주 1회 OFF는 정말 불가피한 경우 뺄 수 있다. 보통 공휴일이 몰린
   주에, 그 주에 법휴가 부여되었다면 OFF 특근이 가능하다.
...
> ⚠️ **확인 대기 (기존 구현과의 차이 — 사용자 확인 전 덮어쓰지 말 것)**:
> ⓐ 토요일 인원: 제1원칙 4는 **4**/3/2, 현재 DB 시드·아래 요일표는 3/3/2.
> ⓑ 제1원칙 4는 "**최소**(결원에도 보장)", 현재 하드 제약은 "정확히 일치(`==`, 초과 불가)".
> ⓒ 제1원칙 3(오프특근)은 현재 하드 제약 "OF 1회/주 정확"에 없는 규칙.
