...
| **완전 확정 표 폴백** | 당월 재적 셀이 사전입력으로 **빈칸 없이** 채워져 있으면, 솔버 infeasible/타임아웃 시 표를 그대로 확정(`pinned_confirmed`) + 규칙 차이는 `pinned_notes` 안내만. 완성 번표는 검증 대상이 아니라 주어진 사실. 완화(allow_pre_relax) 명시 시엔 완화가 우선 |
