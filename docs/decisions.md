...
### 1-23. 연차(V) 설명은 결과 표의 주 단위 산술, 주휴 '이동'은 권하지 않는다 (2026-09-03, M6 P4)
- **결정**: `_v_report`가 솔버가 놓은 V를 주 단위로 "재적 칸 − 근무 = 쉬어야 할 칸 = 주휴 + OF + 확정 휴가 +
  생 + (기타 자동 휴가) + V"로 설명하고 힌트 셋(주휴 없는 사람 · 남는 날 원티드 · 필요 인원 조정)을 낸다.
  결과 표 자체에서 센다 — 요구 인원·야간전담 몫을 다시 계산하지 않아 어긋날 여지가 없다. 사전입력 V는
  사람이 정한 것이라 제외, 야간전담은 OF 규칙 밖이라 제외.
- **이유**: 일별 인원이 '정확히 일치'라 잉여는 반드시 쉬어야 하고 솔버의 휴무 카드는 OF(주1)·생(월1)·V뿐 —
  V는 구조적 잉여의 흔적이다(M6 F8). 사람이 볼 수 있어야 원티드·주휴로 먼저 해결할 수 있다(제1원칙 8).
- **권하지 않는 것**: 감사 계획의 "주휴를 X요일로 옮기면 감소"는 틀렸다 — 주휴는 사람당 주 1회로 총량이
  고정이라 같은 주 안에서 요일을 옮겨도 쉬어야 할 칸 수는 그대로다. V를 줄이는 것은 주휴가 *없는* 사람에게
  넣는 것, 남는 날의 휴가 원티드, 필요 인원 조정뿐. 리포트 문구에 명시했다.
- **테스트 함정**: 테스트 기본 근무 정의는 `공`을 자동 배정으로 열어 두어 잉여가 공으로 흘러 V가 0건이
  된다 — V 관련 테스트는 병동 시드와 같은 근무 정의(`_ward_shifts`)를 명시해야 한다.
...
### 2-22. 'HTML 안 기본 설정'은 안 된다 — 데이터 파일이 유일한 저장소, HTML 은 병동 중립 (2026-09-17)
- **시도**: "json 은 공유용으로만, 초기·기본 설정은 HTML 내부 저장" 요청을 **설정을 구운 HTML 사본 내려받기**로 구현했다
  (`ASSIGN_DEFAULTS` 블록 + `PRISTINE_HTML` 캡처 + `bakeDefaultsHtml`, PR #47). 사용자 지적으로 철회 — 머지하지 않고 닫았고 리포에 흔적이 없다.
- **왜 안 되나**: ① 브라우저는 자기 HTML 파일을 고쳐 쓸 수 없다 — 담을 때마다 3.2MB 사본이 다운로드 폴더에 쌓이고 누군가 공용
  폴더에 손으로 덮어써야 한다. ② 배지가 '다름'으로 재촉해 실제로는 자주 담게 된다. ③ 프로그램 업데이트로 HTML 을 갈아 끼우면
  담긴 설정이 사라진다(리포 원본은 비어 있음). ④ 이 HTML 은 온 병동·입원간호팀 전체가 쓴다 — 병동 값을 HTML 에 넣으면 병동 수만큼
  HTML 이 갈라지고 업데이트가 N배가 된다. ⑤ 인트라넷이라 리포에 설정 JSON 을 두는 대안도 무의미하고, exe 는 인트라넷에서 못 써
  범위 밖이다(사용자 명시).
- **결정**: 데이터 파일(`assign-data*.js`)이 **유일한 저장소** — 병동마다 다른 값은 전부 그 병동의 파일에, HTML 은 어느 병동의 값도
  품지 않는 프로그램. 초기 설정은 시작 안내 체크리스트(병동 고르기 → 병실 14개 자동)로 데이터 파일 안에서 끝낸다. 리포 `baseStore()`
  의 프로그램 기본값(101병동 등)은 '빈 파일의 출발점'일 뿐이다.
- **교훈**: "HTML 안에 저장" 류 요청은 구현 전에 **브라우저가 자기 파일을 못 고친다**는 한계와 사본·업데이트·다중 병동 비용을 먼저
  말하고 방식을 고르게 한다. 여러 병동이 한 프로그램을 쓰는 구조에서 프로그램 파일에 특정 병동 값을 넣지 않는다.

### 2-23. 배포 전제 — 각 병동은 컴퓨터 한 대에 데이터 파일 하나, 공유·동기화 없음, 파일은 백업과 불러오기뿐 (2026-09-17)
- **사용자**: "각 병동은 각자의 한 대의 컴퓨터로 한 개의 파일만 사용할 거야. 공유 폴더에 업로드 후 공유하는 기능은 필요 없어. 그저 백업과 불러오기야."
- **바꾼 것**: 시작 화면·관리 > 데이터 보관·도움말(켜기와 데이터 파일 / 저장과 백업)·투어·처음 안내·온보딩 카드·토스트·저장 메시지에서
  공유 파일·공용 폴더·NAS·다른 PC·여러 병동·exe 를 전부 뺐다(잔존 0 — 스크래치 `test-onepc.mjs` 가 검사). [데이터 파일 열기]는 폴더 고르기
  대신 파일 하나 고르기. **백업 불러오기**(`restoreBackup`)를 새로 두어 백업(또는 데이터 파일)의 내용으로 지금 데이터를 되살리되
  연결된 파일·보관 방식은 그대로 둔다 — 드롭도 연결돼 있으면 되살리기. '이 컴퓨터 안'은 '파일 없이 브라우저 안에 보관'으로 이름을 바꿨다.
- **남긴 것**: rev 충돌 감지(다른 창에서 같은 파일을 고친 경우)는 안전망으로 유지, 문구만 '파일이 밖에서 바뀜'. 사이드카 -2~-5 이름 스캔과
  `openFromDir` 는 옛 배치 호환으로 남김. 팀 전체 확대용 후보 3건(병동 이름 파일·마지막 파일 자동 열기·옆 병동 설정 복사, 세션 노트 6차)은
  **이 전제에서 불필요 — 폐기**.
- **교훈**: 저장 방식 안내문은 배포 전제(몇 대·몇 파일·공유 여부)를 먼저 확정하고 쓴다. 전제가 바뀌면 기능보다 안내문이 먼저 거짓말을 한다.
- **수정 (같은 날, 사용자 지적)**: 위 구현이 전제 문장 "각 병동은 컴퓨터 한 대에 데이터 파일 하나"를 시작 화면·도움말에 **그대로 박았고**, 시키지 않은
  **브라우저 안 보관**을 [파일 없이 시작]으로 전면에 내세웠다. 고침: 전제 문장을 빼고 간호사가 할 일만(파일 한 번 만들기 → 바로 뜸 → [허용] → 백업),
  브라우저 안 보관은 화면·코드에서 제거(옛 잔재 옮기기 `migrateOldLocalData` 만 남김), [처음 시작]이 주 버튼. 교훈 추가: 사용자가 준 **전제는 설계 입력이지
  화면 문구가 아니다**. 시키지 않은 보관 방식을 전면에 두지 않는다. 애매하면 만들기 전에 묻는다.
...
| **근무표가 만들어지면 자동 저장** — 생성 성공·이대로 확정·새로고침 복구 모두. 같은 달 자동저장본은 하나만(덮어쓰기). 손으로 고친 칸은 저장 버튼으로 | "자동저장 붙이자" (2026-09-06) |
...
| **데이터 파일이 유일한 저장소, HTML 은 병동 중립** — 설정을 HTML 에 굽지 않는다. 이 HTML 은 온 병동·입원간호팀 전체가 쓴다. exe 는 인트라넷에서 못 쓰므로 HTML 만 개발 (결정 2-22) | 2026-09-17 "json을 제대로 연결만 해둔다면 html의 한계를 극복할 수 있는 것 아닌가 … 입원간호팀 전체로 확대할거라고" / "Exe 는 인트라넷에서 못 쓰니까 html 로만 디벨롭할거야" |
| **각 병동은 컴퓨터 한 대에 데이터 파일 하나 — 공유 폴더 업로드·공유 기능 불필요, 파일은 백업과 불러오기뿐** (결정 2-23) | 2026-09-17 "각 병동은 각자의 한대의 컴퓨터로 한개의 파일만 사용할거야. 공유폴더에 업로드 후 공유하는 기능은 필요없어. 그저 백업과 불러오기야" |
| **사고 과정과 답변은 한국어로** — 한국어로 지시하면 보이는 생각도 한국어로 쓴다 | 2026-09-17 "내가 한국어로 지시하면 당연히 한국어로 대답하고 사고과정도 한국어로 보여줘야 하는 거 아니야?" |
| **설계 전제 ≠ 화면 문구** — "각 병동은 컴퓨터 한 대에 파일 하나" 같은 전제를 안내문에 그대로 쓰지 않는다. 간호사 관점에서 할 일만. 시키지 않은 방식(브라우저 안 보관)을 전면에 두지 않는다. 헷갈리면 만들기 전에 묻는다 | 2026-09-17 "각 병동은 컴퓨터 한 대에 데이터 파일 하나 기준으로 만들라고 했지 그걸 그대로 갖다 붙이랬냐? … 브라우저 안에 파일 통합하는 걸로 내가 하라 그랬니? 그냥 json 방식으로 고도화하자고 했잖아 … 헷갈리는 게 있으면 물어보던가" |
