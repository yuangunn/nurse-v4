...
### 1-16. 오프특근 — 그 주 공휴일에 법휴·근무가 있는 사람만 OF 면제 (2026-08-20 확정, 같은 날 조임)
- **결정(현행, v4.9.0)**: 주별 OF는 `≤1` **상한 + 1회 의무**인데, 그 의무는
  **그 주 공휴일에 `법`을 받았거나 근무한 사람에게 면제**된다. 모델에서는 사후
  판정이 아니라 **선형 함의**로 건다 (자유 셀의 근무·법휴도 근거가 되어야 하므로):

      Σ_{주} OF + Σ_{그 주 공휴일} (법 + 모든 근무코드) ≥ 1,   Σ_{주} OF ≤ bound

  → 연휴 내내 근무도 법휴도 없이(주휴·연차 등으로만) 지낸 사람은 OF 1회 의무가
  그대로다. 사후 판정용 헬퍼는 `_week_off_exempt(nurse, week_days, codes=None)`
  (결과 표 또는 사전입력 기준), 선형식 항목은 `_week_off_exempt_shifts(week_days)`.
- **경위**: v4.8.0은 "공휴일이 낀 주면 누구나 `≤1`"로 단순화해 출시 → 같은 날
  후속 리뷰에서 **원문보다 넓다**(그 주에 쉬지도 일하지도 않은 사람까지 면제)는
  점을 사용자가 확인해 조였다.
- **왜 '법휴 only'가 아닌가**: 처음엔 원문의 "법휴가 부여되었다면"만 근거로 삼아
  구현했는데, 시뮬(2026-09 추석, 18명)에서 **법휴를 7~10명에게 부여해도 그 달이
  안 풀렸다**(CP-SAT UNKNOWN/infeasible). 오프특근의 실체가 "공휴일에 특근했으니
  그 주 OFF를 못 받는다"이므로 **공휴일 근무도 근거**에 포함하는 것이 맞고,
  사용자 승인 문구도 "법휴/공휴일 근무가 있는 사람"이었다. 이 조합에서 연휴 달이
  정상 통과한다.
- **안내**: 연휴 주가 빡빡할 때를 대비해 세 곳에 안내를 둔다 — 진단 Phase 5
  (공휴일 주별 면제 대상 인원 + "공휴일에 '법'을 넣어 쉴 사람을 정해 주세요"),
  분석 탭 참고 경고(공휴일 주 목록 + 법휴 인원), `conflict_analyzer` 라벨
  ("공휴일 주 — 법휴·공휴일 근무 시 면제"). 시뮬레이터도 병동처럼 공휴일에
  법휴를 부여한다(`beop_pins`).
- **이유**: 공휴일 OF 금지 × OF 주1회 정확 × 야간전담 5일 윈도우가 겹치면 연휴
  주의 비공휴일 캐파가 만석이 되는 구조적 교착 (2026-09 추석 시뮬로 증명).
  병동 현실의 탈출구가 오프특근이며, ~~주휴 공휴일 배치~~는 존재하지 않는 관행.
- **파일**: `scheduler_base._week_has_holiday`/`_week_off_exempt`(공용 판정) +
  `_c_weekly_off`/`_cs_weekly_off`(양 엔진) + `conflict_analyzer._g_weekly_off` +
  `_pinned_rule_notes` ③ + `scheduler_highs_diagnosis` Phase 5 안내·Phase 8 주간
  산식(`required_off`) + 프론트 `analysis.js`/`grid-interactions.js`.
  테스트 `test_constraints.py` 오프특근 **2종×양엔진**(법휴 있으면 OF 0 허용 /
  법휴 없으면 여전히 infeasible — 규칙이 다시 헐거워지면 깨진다).
...
- **이유**: 사실-클램프(1-15) 이후 엔진은 확정 셀의 규칙 위반을 **사실로 수용**한다
  ...
  **🟢 신호등 + 빨간 경고 벽**이 동시에 떴다. 제1원칙 7("사전입력에는 사유가 있다")과도
...
| **병동 도메인 제1원칙 7건** — 공휴일 운영·오프특근·최소 인원(토 4/3/2)·주휴 공휴일 배치 관행 없음(중복수당)·외과병동 구조·원티드의 의미(사유 있음, V 대량 자동 사용 불가) | 2026-08-20 명시 — **CLAUDE.md 제1원칙 절이 원문**. 모든 세션 작업 전 필독 |
| **잉여 인원 처리 = 연차·휴가로 소진** — 일별 인원 `==`(정확 일치)는 현행 유지, 초과 출근 없음 | 2026-08-20 확인 질문 답변 |
| **오프특근은 근거 있는 사람만** — 그 주 공휴일에 법휴를 받았거나 근무한 사람만 OF 면제 (결정 1-16) | 2026-08-20 확인 질문 답변(단순화)로 v4.8.0 출시 → 같은 날 "법휴/공휴일 근무가 있는 사람으로 한정" 승인 → v4.9.0 |
| **사전입력 경고는 훈계가 아니라 참고** — 확정 셀만으로 결정되는 항목은 빨강 위반이 아니라 회색 참고. 빨강은 '알 수 없는 코드'뿐 (결정 1-18) | 2026-08-20 파인튜닝 항목 1 — 제1원칙 7(사전입력에는 사유가 있다)의 UI 반영 |
