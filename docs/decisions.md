...
| **생휴(생)는 보장이 아니다** — "월초부터 월말까지 1회 주어질 **수 있다**"일 뿐, 스케줄상 안 나오면 못 받는 것이고 어쩔 수 없음. 강제 배정 금지 (월 ≤1 상한만) | 2026-08-19 "생휴는 무조건이 아닌데도… 그 부분을 잘 기억해줘" — 본 제약 `_c_menstrual_leave`는 이미 ≤1로 부합. **예외 1곳**: 야간전담 여성+31일달 '정확히 1회' 강제(`_c_night_shift_nurses` #4)가 이 원칙과 상충 가능 — 유지/제거는 사용자 확인 필요 |
