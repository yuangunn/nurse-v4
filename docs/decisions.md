...
### 2-24. 13호 유무는 병동이 고른다 — 기본값은 '없음', 켜면 12호 보는 자리에 붙인다 (2026-09-17)

**배경**: 기본 프리셋(101병동 기준)에는 13호가 없는데 병동을 고르면 병실 14개가 생겨 13번 자리(63호 등)를 아무도 안 보는 빈틈이 있었다 (실사용 전수조사).
사용자: "13호가 있는 곳이 있고 없는 곳이 있어 자율로 결정하게 해줘."

**결정**: `store.has13` 을 병실 목록의 [13호 있음] 체크 하나로 정한다. 기본값은 **없음**(기존 기본 병실 목록과 같게). 켜면 병실 목록에 넣고 모든 프리셋의
'12호 보는 자리'에 붙이고(`attach13`), 끄면 병실·프리셋·주지 않을 방 어디서나 뺀다. 옛 파일은 병실 목록에 13호가 있는데 어느 프리셋도 모르면 열 때 붙여 준다.
같은 조사에서 나온 나머지 항목(헤더 탐색·확인 카드 확장·픽커·근무 입력 모달·전출·파일 안 스냅샷 등)은 세션 노트 2026-09-16 10차와 CLAUDE.md 어싸인 절 참조.
...
| **병실은 병동마다 있고 없고가 다르다 — 특정 호실(13호)을 특별 취급하지 않는다. 병동을 고르면 늘 14개를 만들고, 없는 방은 ✕·더 있으면 ＋로 병실 목록에서 고친다 (13호도 기본으로 있음).** 프리셋은 병실 목록의 모든 방을 한 자리씩 덮어야 한다 (결정 2-24, 2026-09-18 개정) | 2026-09-17 "13호가 있는 곳이 있고 없는 곳이 있어 자율로 결정하게 해줘" → 2026-09-18 "13호실은 있는게 default 값이야. 13호실 만 특별한게아니라 병동마다 병실이 있을수도 있고 없을 수도 있어" |
| **한 병실을 둘이 나눠 볼 수 있다 (병상 단위 배정)** — 환자가 많을 때 공정성을 위해. 토큰은 방(`1001`) 또는 병상(`1001:1`), 화면은 `1` / `1:1`, 병상 번호는 병상수로 자동. **배정 코어에는 늘 병상 키로 펼쳐 넘긴다** — 표기가 섞이면 같은 환자인데 겹침이 0이 되고, 펼치면 겹침이 곧 '같이 보던 환자 수'라 코어를 고칠 필요가 없다 (결정 2-26) | 2026-09-18 "병상으로 배정도 가능하게 해줄수있어? … 환자가 많은경우 공정성을 더하기위해 한 병실을 둘이 나눠서 보는거지" — 표기 `1:1` 한 자리·번호 자동·주지 않을 방도 병상 단위로 결정 |
| **병실 목록은 전체 호실(1001호)로 보여주고 순서를 바꿀 수 있어야 한다** — 배정표 표기(1호)는 따로 보여준다 | 2026-09-18 "병실 목록에서 수정할때 모든 호실 (ex 1001호)이 보이게 해주고 순서 수정도 할 수 있게 해줘" |
| **양식 xlsx 는 빈 양식에서 뽑는다** — 데이터가 든 시트를 지워 쓰면 그 주에만 칠한 서식(행사 칸 바탕색)이 딸려 들어온다. 인쇄 HTML 도 양식에 없는 색을 하드코딩하지 않는다 | 2026-09-18 "배정표 양식 뽑으랬더니 특정 시트에 있는 특별한 서식(해당일에만 적용되는 배경색)을 써버리면 어떻게해" — 병동이 준 빈 양식으로 교체 |
| **배정표 출력 서식은 병동이 고른다 (101·122 두 벌 내장, 각각 문구·양식 교체 가능)** — 서식마다 칸 구조가 다르다(101=요일마다 방·이름 / 122=요일마다 이름·메모 + 방은 일·월~금·토 열에 한 번). 자리 이름도 서식을 따른다(A(CN)·B·C·D·E / CN·A·B·C·D, 야간 A·B·C). 병동 번호로 자동 판별하지 않는다 (결정 2-25) | 2026-09-17 "배정표 출력서식을 101병동으로 할지, 12x병동으로 할지 선택할 수 있게 해주고 각 서식 모두 수정할 수 있게 해줘" |
