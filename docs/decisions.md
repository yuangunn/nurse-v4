...
### 1-16. 오프특근 — 공휴일 포함 주는 OF ≤1 (2026-08-20 확정)
- **결정**: 제1원칙 3(주 1회 OFF는 불가피하면 뺄 수 있음 — 오프특근)을 조건
  단순화로 구현: **당월 공휴일이 하나라도 포함된 완전한 주는 weekly OF를 `==1`이
  아니라 `≤1`(0회 허용)**로 완화. 사용자 답변으로 조건 확정 (법휴 부여자 한정
  대신 주 단위 단순화 채택).
...
- **파일**: `scheduler_base._week_has_holiday`(공용 판정) +
  `_c_weekly_off`/`_cs_weekly_off`(양 엔진) + `conflict_analyzer._g_weekly_off`
  (라벨 "OF ≤1 (공휴일 주 오프특근)") + `_pinned_rule_notes` ③(공휴일 주 OF 0회
  안내 제외) + 프론트 `analysis.js`/`grid-interactions.js` 경고 가드.
  테스트 `test_constraints.py` 오프특근 2건×양엔진.
...
| **오프특근 구현 (조건 단순화)** — 공휴일 포함 완전한 주는 OF ≤1 (결정 1-16) | 2026-08-20 확인 질문 답변 |
