...
## M4. "사전입력 인원 딱 맞는데 생성 실패" 근본 원인 — ✅ 조사 완료 (2026-08-18) · 개선 구현 대기
...
- [ ] **P1 (저비용·고효과) 진단 개선**: Phase 5(주휴/OF)에 주간 휴무 산술 검사 추가 —
...
- [ ] **P4 (이월/붙여넣기 실용 개선)**: 붙여넣기·업로드에서 **한 주에 OF가 2개면
      하나를 '주'로 자동 변환** 제안(미리보기에서 확인 후 적용). 주휴 구분이 없는
      병동식 표를 그대로 받게 하는 가장 값싼 다리. 위치: `paste-import.js` 적용 직전
      검사 + 모달 안내.
- [ ] **P5 (정책 — 사용자 결정)**: '전월 기록 존중 모드' — 경계 걸침 4종 제약에서
      **전월 쪽 셀이 사전입력 확정인 경우** 해당 제약을 경고로 낮추는 토글.
      반론: 경계 검증이 확장 구간의 존재 이유(N→D 등 실제 위험 방지)이며 진단이
      이미 셀 좌표를 정확히 짚어줌 → 기본 OFF 유지가 안전.
- [ ] **P6 (사용자 확인 필요)**: 야간전담 여성+31일달 '생 정확히 1회' 강제
      (`_c_night_shift_nurses` #4)가 2026-08-19 사용자 원칙("생휴는 보장 아님 —
      못 받아도 어쩔 수 없음", decisions.md 사용자 규칙)과 상충 가능. 유지/제거 결정.
