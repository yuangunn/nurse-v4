    ...
    def _diagnose_phases(self) -> str:
                    ...
                    for n in regular_nurses:
                        ...
                        for d in week_dates_idx:
                            ...
                            if pre in day_cover or pre in evening_cover:
                                ...
                                pre_busy_non_off += 1
                        # 의무 휴무: 주휴 1만 센다. OF 주 1회는 오프특근(제1원칙 3)
                        # 으로 양보될 수 있으므로(슬랙) '공급 부족'을 계산할 때 의무로
                        # 세면 있지도 않은 부족을 보고하게 된다.
                        duty = 1
        ...
        if not _try(p):
            ...
            if issues:
                ...
                lines.append("    이 단계까지 쌓았을 때 처음 막힌 것일 뿐, 대개 원인은 생휴가 아니라 '쉴 수 있는 코드의 공급 부족'입니다:")
                lines.append("    솔버가 놓을 수 있는 휴무는 OF(주당 정확히 1회)·V(월 1회)·생(월 1회, 여성)뿐이라,")
                lines.append("    빈 칸(잉여 인력)이 그보다 많으면 채울 코드가 없어 실패합니다.")
                lines.append("  → 해결: 주휴(주)를 사전입력하거나(분석 탭 → 주휴 배분 → 사전입력에 적용), 간호사 수·필요 인원 균형을 조정하세요.")
            return "\n".join(lines)
