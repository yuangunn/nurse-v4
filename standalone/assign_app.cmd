...
set "OPT=--app=%URL% --user-data-dir=%PROF% --no-first-run --no-default-browser-check"

where msedge >nul 2>nul && (start "" msedge %OPT% & exit /b)
if exist "%ProgramFiles(x86)%\Microsoft\Edge\Application\msedge.exe" (
  start "" "%ProgramFiles(x86)%\Microsoft\Edge\Application\msedge.exe" %OPT% & exit /b)
if exist "%ProgramFiles%\Google\Chrome\Application\chrome.exe" (
  start "" "%ProgramFiles%\Google\Chrome\Application\chrome.exe" %OPT% & exit /b)
